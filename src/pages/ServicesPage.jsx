...
const SERVICES = [
  { title: 'Residential Construction', desc: 'We deliver end-to-end construction with strong foundations and finishes.' },
  { title: 'Architectural Design', desc: 'Smart architectural planning and space optimization tailored to client needs.' },
  { title: 'Contracting Services', desc: 'Reliable project management with precise material sourcing and execution.' },
  { title: 'Sustainable Building', desc: 'Eco-friendly methods creating innovative, healthier, and future-ready living spaces.' },
]
...
export default function ServicesPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-16 md:py-24">
      ...
      <p className="mt-6 font-body text-text-dark/80 max-w-3xl">
      ...
      <div className="mt-12 grid lg:grid-cols-2 gap-12">
        <div>
          <h2 className="font-heading text-xl font-semibold text-text-dark mb-4">Our Services</h2>
          <div className="space-y-2">
            {ACCORDION_ITEMS.map((item, i) => (
              <div key={item.title} className="rounded-lg border border-brand/20 overflow-hidden">
                <button
                  type="button"
                ...
                <div className={`overflow-hidden transition-all duration-300 ${openIndex === i ? 'max-h-40 opacity-100' : 'max-h-0 opacity-0'}`}>
                  <p className="px-5 pb-4 pt-0 font-body text-text-dark/80 text-sm">{item.content}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          {SERVICES.map((card) => (
            <div key={card.title} className="rounded-xl border border-brand/20 bg-brand-light/20 p-5 hover:border-brand/40 transition-colors">
              <h3 className="font-heading font-semibold text-text-dark">{card.title}</h3>
              <p className="mt-2 font-body text-sm text-text-dark/80">{card.desc}</p>
            </div>
          ))}
        </div>
      </div>
